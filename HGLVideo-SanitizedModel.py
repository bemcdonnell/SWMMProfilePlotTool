...
sys.path.append('C:/PROJECTCODE/SWMMOutputAPI')
...
OutputObject.OpenBinFile('./TestModel/SanitizedModel.out')
SWMMNodes = OutputObject.get_NodeIDs()
NPeriods = OutputObject.get_Times(numPeriods)        
...
for ind,ID in enumerate(MHList):
    RESULTMATRIX[ind,:]=list(OutputObject.get_NodeSeries(SWMMNodes[ID],hydraulic_head))
    
if len(LKsToPlotAppendix)>0:
    RESULTMATRIXAppendix = np.zeros((len(MHListAppendix),NPeriods ))
    for ind,ID in enumerate(MHListAppendix):
        RESULTMATRIXAppendix[ind,:]=list(OutputObject.get_NodeSeries(SWMMNodes[ID],hydraulic_head))  
